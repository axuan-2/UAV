...
//  float step_mapping=0,scale_mapping=0;
  ...
  ********/
  ...
  {
    //油门杆上推、给定速度期望
    Total_Controller.High_Speed_Control.Expect=RC_Data.rc_rpyt[RC_THR];//最大上升速度50cm/s
    handmove_flag=TRUE;
    move_flag=1;
  }
  else if(RC_Data.rc_rpyt[RC_THR]<0)//给定下降速度期望
  {
    Total_Controller.High_Speed_Control.Expect=RC_Data.rc_rpyt[RC_THR];//最大下降速度40cm/s
    handmove_flag=TRUE;
    move_flag=2;
  }
  /*高度控制器第2步*/
