...
//  }  
	  ...
	  if(PPM_Databuf[6]>=(RC_Data.cal[6].max-RC_Data.cal[6].deadband))            {Controler_Land_Mode=2;}//???? //7????
  else if(PPM_Databuf[6]<=(RC_Data.cal[6].middle+RC_Data.cal[6].deadband))    {Controler_Land_Mode=1;}//?????

  if(PPM_Databuf[6]>=(RC_Data.cal[6].middle-RC_Data.cal[6].deadband)
   &&PPM_Databuf[6]<=(RC_Data.cal[6].middle+RC_Data.cal[6].deadband))//???????????     
  {
   
  }
  else if(PPM_Databuf[6]<=(RC_Data.cal[6].min+RC_Data.cal[6].deadband))  //7????  
  {
    stop_oar_flag=1;
  }
...
void Control_Output()
{
	...
	if(stop_oar_flag==1)
	{
		PWM_Output((uint16_t)(1000),(uint16_t)(1000),(uint16_t)(1000),(uint16_t)(1000)/*,0,0,0,0*/);
  ...
  if(Controler_State==Unlock_Controler)//解锁
  {
    ...
    {
      if(Last_Motor_PWM_1<=Thr_Min&&Last_Motor_PWM_2<=Thr_Min&&Last_Motor_PWM_3<=Thr_Min&&Last_Motor_PWM_4<=Thr_Min)//只有上锁后再解锁时才会满足
      {
        //如果上次油门输出值为最低位，进入怠速时，安排过渡过程
        Thr_Idle_Transition_Cnt=Idel_Transition_Period;
      }
			else if(SDK_finish_flag==1)//其他时刻进入着陆条件
      {
        if(Last_Landon_Earth_Flag==0)//上次为起飞状态，本次为着陆状态，上锁电机
        {
          ...
          Bling_Set(&Light_3,3000,200,0.5,0,GPIO_PORTF_BASE,GPIO_PIN_3,0);
			...
			else if((SDK1_Mode_Setup==16)||((SDK1_Mode_Setup==13)&&(Delay_Transform_Time==160)))
			{
				PWM_Output((uint16_t)(1000),(uint16_t)(1000),(uint16_t)(1000),(uint16_t)(1000)/*,0,0,0,0*/);
			}
...
//    Throttle_Control_Reset();
  ...
  Motor_PWM_4=Value_Limit(0,2500,Motor_PWM_4);
	PWM_Output((uint16_t)(1.25*Motor_PWM_1),(uint16_t)(1.25*Motor_PWM_2),(uint16_t)(1.25*Motor_PWM_3),(uint16_t)(1.25*Motor_PWM_4));
}
